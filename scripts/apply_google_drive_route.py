#!/usr/bin/env python3
"""Install ST-16 Google Routes overseas tool and attach it to qualified public text.

Does not change Pipe valves. Merges tool valves so an existing GOOGLE_MAPS_KEY is kept.
Key sources: env GOOGLE_MAPS_KEY / GOOGLE_ROUTES_KEY / GOOGLE_MAPS_API_KEY, else leave current valve.
"""
...
TOOL_SOURCE = Path(__file__).with_name("google_drive_route_tool.py")
TOOL_NAME = "Overseas Drive Route"
...
def tool_source() -> str:
    ...
    if GOOGLE_DRIVE_ROUTE_MARKER not in content:
        raise RuntimeError("google routes tool source missing marker")
    return content
...
def upsert_tool(h: dict[str, str]) -> dict[str, Any]:
    ...
    if status == 200 and existing:
        response = requests.post(
            f"{OPENWEBUI_URL}/api/v1/tools/id/{GOOGLE_DRIVE_ROUTE_TOOL}/update",
            headers=h,
            json=payload,
            timeout=120,
        )
    ...
    grants = requests.post(
        f"{OPENWEBUI_URL}/api/v1/tools/id/{GOOGLE_DRIVE_ROUTE_TOOL}/access/update",
        headers=h,
...
def merge_valves(h: dict[str, str]) -> dict[str, Any]:
    ...
    existing_key = str(current.get("GOOGLE_MAPS_KEY") or "").strip()
    valves = {
        "GOOGLE_MAPS_KEY": env_key or existing_key,
        "MAX_CALLS_PER_TURN": int(current.get("MAX_CALLS_PER_TURN") or 3),
        "MAX_VIA_POINTS": int(current.get("MAX_VIA_POINTS") or 24),
    }
    response = requests.post(
        f"{OPENWEBUI_URL}/api/v1/tools/id/{GOOGLE_DRIVE_ROUTE_TOOL}/valves/update",
        headers=h,
        json=valves,
        timeout=30,
    )
    ...
    print(f"tool valves key_set={bool(valves['GOOGLE_MAPS_KEY'])} max_calls={valves['MAX_CALLS_PER_TURN']}")
    return valves
...
def attach_tool(h: dict[str, str], model_ids: list[str], *, attach: bool) -> None:
    ...
    for model_id in inspect:
        ...
        if model_id in wanted:
            if GOOGLE_DRIVE_ROUTE_TOOL not in tools:
                tools.append(GOOGLE_DRIVE_ROUTE_TOOL)
                changed = True
        elif GOOGLE_DRIVE_ROUTE_TOOL in tools:
            tools = [tid for tid in tools if tid != GOOGLE_DRIVE_ROUTE_TOOL]
            changed = True
        if not changed:
            continue
        if tools:
