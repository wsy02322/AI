#!/usr/bin/env python3
"""Temporary stream status for Filter F1 (Astra vs Sol). Content-only.

Emits filter_probe from both __metadata__ and body after
_apply_server_tools_metadata. Does not change tool gates, valves, or API_KEY.
Always --revert when the probe is done.
"""
...
MARKER = "ASTRA_SEARCH_FILTER_PROBE_V1"
...
SNIPPET = ANCHOR + f'''        # {MARKER}
        if __event_emitter__:
            try:
                ...
                _fp = _pm.get("filter_probe") if isinstance(_pm, dict) else None
                _bmeta = body.get("__metadata__") if isinstance(body, dict) else None
                _bpm = (_bmeta or {{}}).get(_PIPE_METADATA_KEY) if isinstance(_bmeta, dict) else None
                _bst = _bpm.get("server_tools") if isinstance(_bpm, dict) else None
                _bfp = _bpm.get("filter_probe") if isinstance(_bpm, dict) else None
                ...
                _desc = (
                    ...
                    f"bst={{list(_bst) if isinstance(_bst, dict) else _bst}} "
                    f"fp={{_fp}} bfp={{_bfp}} "
