#!/usr/bin/env python3
"""C2 smoke: DeepSeek / Kimi / Qwen Search+Fetch, plus one continue.

Writes /opt/cursor/artifacts/search-cost-china.json. Does not change valves.
"""
...
from stack_contract import CHINA_TEXT_MODEL_IDS, PIPE
...
OUT = Path(os.environ.get("SEARCH_COST_CHINA_OUT", "/opt/cursor/artifacts/search-cost-china.json"))
CONTINUE_LIMIT = 8
CONTINUE_MODEL = CHINA_TEXT_MODEL_IDS[1]  # Kimi
...
def main() -> int:
    ...
    for model_id in CHINA_TEXT_MODEL_IDS:
    ...
    first = _search(h, CONTINUE_MODEL)
    ...
    rows[f"{_short(CONTINUE_MODEL)}.continue"] = {
        **_summarize(continue_turn),
        "limit": CONTINUE_LIMIT,
        "first_search_requests": web_search_requests(first.get("usage")),
    }
    print(
        f"{_short(CONTINUE_MODEL)} continue status={continue_turn['status']} "
        f"web_search_requests={continue_searches} "
        f"cost={usage_cost_usd(continue_turn.get('usage'))}"
    )
    if continue_turn["status"] != 200:
        errors.append(f"{_short(CONTINUE_MODEL)} continue {continue_turn['status']}")
    elif continue_searches > CONTINUE_LIMIT:
        errors.append(
            f"{_short(CONTINUE_MODEL)} continue web_search_requests={continue_searches} > {CONTINUE_LIMIT}"
        )
    ...
    print(f"wrote {OUT}")
    print(f"china smoke: {len(rows) - len(errors)} ok, {len(errors)} err")
    for error in errors:
        print(f"  - {error}")
