...
**工作源 = 本目录拆分文件**（`AGENTS.md`、`SPEC.md` 等）。它们与本仓库根上的 Open WebUI `AGENTS.md` / `docs/SPEC.md` **不是同一套产品**，不要合并。

## 单文件下载（整包复制到新仓）

- `HANDOFF_BUNDLE.md` — 本目录文件的 **打包快照**（按 `===== FILE: ... =====` 分段）。改规范时先改拆分文件，再视需要重生成 BUNDLE。不要只改 BUNDLE。
