...
**P0 四条并列**：图像生成、**语音聊天**、**屏幕共享**、Notebook/YouTube（各有独立 plan）。语音与屏享同级，不得写成「屏享 → 语音」；两者都受宪法复杂度确认门约束。视频生成与 slides 仍为 Later 必做，**不是** YouTube 知识理解。维持 **23 个 public**（原则：顶级 + 轻量 + 中国约两个顶级文本；名单见 `PUBLIC_MODEL_IDS`。Astra 一对已入 **且挂 ST-14**）。
...
| `scripts/run_text_web_search_smoke.py` | ST-14 W3：9 个 allowlist 模型 Search + Fetch |
