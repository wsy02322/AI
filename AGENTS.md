...
**ST 编号**：**ST-11** = Fable 同模型续聊（unsigned thinking）；**ST-12** = Follow-up 芯片关；**ST-14** = 指定文本模型薄 Web Search；**ST-16** = 出行路线工具（W1 高德 / W4 Google Routes / **M1a via+legs** / **MAP_LITE 表+导航+一张高德静态图**）；**ST-17** = Grok native / X（W3 已过门；W5 官方 X 按次已过门）。不要把这些写成同一个号。
