...
| 搜索 | 指定 9 个文本模型挂薄 `Web Search`（ST-14；原 7 + Astra 一对）。两档 Sonar 仍是 Quick / Deep。原生 Web Search **关**。broad OR Web Tools **停用**。已知限制：Anthropic 读不了 `api.github.com` Releases JSON |
...
Sonar / 纯图像：`code_interpreter=false`、`web_search=false`、`builtin_tools=false`；纯图像另 `terminal=false`。filterIds 含 `openrouter_direct_uploads`，图像另加对应 `openrouter_image_filter_*`。指定 9 个文本模型（原 7 + Astra 一对）另挂 `openrouter_text_web_search`（default-on）。**不要**挂 `openrouter_web_tools` / `openrouter_image_gen`。
...
10. `python3 scripts/apply_text_web_search.py --mode final`（薄 Web Search 挂 9 个文本模型并 default-on）。  
