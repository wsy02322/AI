...
10f. `python3 scripts/apply_search_quality_m1a.py`（ST-16 M1a + MAP_LITE：`via` + `legs[]` + 导航链接 / 中国多站一张高德静态图；merge Valves，不覆盖 Key）。  
