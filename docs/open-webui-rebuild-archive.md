...
6. 按任务再读：图像 continuity、Live、Notebook、文件录入、**0.11.3 升级**（`docs/open-webui-upgrade-0113-plan.md`）、搜索账单（`docs/open-webui-search-cost-plan.md`）、即时搜顶级档（`docs/open-webui-search-quality-top-plan.md`：W0 已测、**W1 高德已过门**、**W2 Gemini native 已过门**、**W3 Grok native 已过门**、**W4 Google Routes 已过门**、**W5 X 按次已挂、过门等 Key**）  
...
| 搜索 | **12** 个 public 文本挂薄 `Web Search`（ST-14）：OpenAI **Exa**，**Google native**（W2），**xAI native**（ST-17），Anthropic/中国三只 `auto`。Tool `amap_drive_route` + `google_drive_route`（ST-16；中国高德 / 海外 Google Routes）+ `x_recent_search`（ST-17 / W5；官方 X 近 7 天，过门等 Bearer）。两档 Sonar 仍是 Quick / Deep。原生 Web Search **关**。broad OR Web Tools **停用**。已知限制：Anthropic 读不了 `api.github.com` Releases JSON |
...
`verify_stack.py` 验 Banner v7、suggestions=0、Follow-up 关、Fable marker、picker=`ACTIVE_MODEL_IDS`（23）、ST-14 薄 Web Search + ST-16 `amap_drive_route` / `google_drive_route` + ST-17 `x_recent_search`（12 个 public 文本，含 Astra 与中国三只）。不要为了绿把 Banner 改回 v6/v5/v4/v3/v2。
...
**决策（勿擅自改回）**：方案 A 藏 broad Web Tools 三件套；指定文本模型走薄 ST-14 Web Search；Sonar 仍是 Quick / Deep；不装第二套 Pipe；界面英文；Banner 不可 dismiss；`openai.api_configs` 全 disable；L0 不持久化 JWT。
