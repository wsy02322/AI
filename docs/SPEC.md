...
4. **Walkthrough 证据**：除非特别必要，**不要**录屏或截屏；优先自动化验收（`verify_stack` 等）与终端/日志输出。特别必要 ≈ 纯 UI 改动且脚本无法覆盖、或用户明确要求可视化证据。
...
| UX-2 | 英文指引：两条 Banner + 关键 Description。蓝条只强调 **Sonar = 联网、图像模型 = 作图**（无语音/屏享/Notebook、无 Sol/Opus Chat 句）。**空对话 Suggested 清空** |
