...
| UX-2 | 英文指引：**一条** Banner `usage-guide-v7`（🌐 Text chat models can search the web and read pages；🔗 GitHub 用 github.com 不用 api.github.com；🖼️ 图像只走图像模型；🧠 Valves Reasoning depth；📝 System Prompt 会影响图像与 Perplexity sonar；同一段、句首图标、无粗体）+ 关键 Description；**无**空对话 chips（OWUI 点击即发送） |
| UX-3 | Integrations **无** OR Web Tools / OR Image Gen / OWUI native Web Search；**public 文本**可有薄 **Web Search**（现网 12 个，含 DeepSeek / Kimi / Qwen；图像 / Sonar / 视频不得挂）；保留 Direct Uploads；图像模型可有 native image filter |
| UX-4 | **23 个 public**（对比用；**不必守 21**）。**原则**：最顶级 + 若有的轻量版 + 中国约两个顶级文本（现网中文文本实际是 DeepSeek / Kimi / Qwen 三个，这次不拿掉）。留下的家族只跟 catalog **最新 id**，且 **全部 public**。含 `claude-fable-5.1`、`gemini-3.1-pro-preview`、`gemini-3.8-flash`、`qwen3.8-max-0902`，以及 `openai.gpt-6-astra` / `openai.gpt-6-astra-pro`（能选、能聊、**挂 ST-14**）。`granite-4.2-8b`、`mercury-2.5-preview` 及未确认新家族（Ling / Muse / Hailuo / GLM Flash 等）**不** active。契约外模型不得带 `*` read |
...
| ST-14 | **public 文本**挂非 global 薄 Filter `openrouter_text_web_search`（显示名 Web Search）：inlet **deny 类**（Sonar / `image_output` / `video_generation`），无模型 allowlist。写入 OpenRouter `server_tools` Search + Fetch，新对话 default-on，用户可关。循环停止：非 OpenAI 为 `step_count_is=8` 或 `$0.05`；**OpenAI 走 native + `max_tool_calls=3`**（W6，去掉会被覆盖的 stop）。OpenAI 400「Server tool request failed」：先剥回放图再关 `openrouter:` server tools 内部重试（`SERVER_TOOL_FAIL_RETRY_V1`；不新开 ST）。**Google 走 native**（W2）；**xAI 走 native**（ST-17 / W3，网页+X）；Anthropic 留 `auto`。中国三只 `auto`（回落 Exa）。挂载后必须 `GET /api/models?refresh=true`。**未确认**不上 Search Controller、不加 Filter 指引、不抬 `$0.05`。评测见 `docs/open-webui-text-web-search-eval-b-results.md`（原 7 个收口；中国三只以 Search+Fetch 烟雾为准）。**不要**把本条和 ST-11/ST-12 写成同一个号 |
| ST-16 | 合格 public 文本挂 `amap_drive_route`（中国）和 `google_drive_route`（海外）：压缩 JSON，每轮最多 3 次。可选 `via`（最多 6 站）回 `legs[]` + `totals`（**M1a 已过门**）。多站中国线：每段一条高德网页导航 + HTTPS 全程落地页 `nav_page_url`（页上再开 App；**不出**聊天里的 `amapuri`）+ **一张**沿路网折线的高德静态图（`map_data_uri`；**MAP_LITE / MAP_ROAD / NAV_LEGS / NAV_WEB_ONLY / NAV_PAGE**）。海外只要 Google 链接。无交互地图 / 电话 / 评分。失败说「路线接口不可用」。钥匙只在 Tool Valves，禁止回放到聊天。**W1 / W4 / M1a 已过门**。**M1b**（偏高速/少收费）未确认。不要和 ST-11/12/14 混号 |
| ST-17 | xAI 类薄 Filter Search+Fetch **`engine=native`**（网页+X）。OpenAI 走 native + `max_tool_calls=3`（W6，记在 ST-14）。Google 走 native（W2，记在 ST-14）。Grok 须能引用 x.com。**W5 已过门**：官方 X API 按次薄工具 `x_recent_search` 挂 12 个 public 文本；Key 在 Valves。失败说「X 接口不可用」，禁止编 `x.com/status`。不要和 ST-11/12/14/16 混号 |
...
- **已落地**：聊天四格 + 路线 S；ST-14 薄 Web Search（质量**已收口**；**W2 Google native / W6 OpenAI native 已过门**）；Live **L1**；**ST-16 高德 + Google Routes + M1a via/legs + MAP_LITE（表+每段网页导航+全程落地页+一张高德静态图；聊天不出 App 深链）**；**ST-17 Grok native / X 已过门**；**W5 `x_recent_search` 已过门**  
...
- **Later（须单独确认）**：新 public 文本按类挂能力（C2 已落地：搜索 deny Sonar/图像/视频）；ST-14 **Search Controller**（Fetch 失败重试 / GitHub API→HTML）；Wave 1 **视频生成**（至少 1 个旗舰 public 且实测出片；聊天模型不得因 video tool 404）；Wave 2 slides（独立入口，聊天主路径无新 tool）；对比 **S3 真并行分栏**（ST-10 重试已落地，S3 未做）；Notebook N3/N4 Studio；**文件上传对标官网最高档**（见 `open-webui-file-ingest-plan.md`，T0 未确认）；关 OWUI 图像模型 / Studio A4（多参考、流式）；**M1b**（偏高速/少收费）  
- **Don't**：ComfyUI、第二套 Pipe、重开 Web Search 三件套、466 全 public、同会话作图主路径、L3 三家 Live 并行、stock+realtime 双容器、把 RAG 当加分项、把 YouTube 转录当成 NotebookLM 达标、用 gpt-audio 冒充已接好的 Call S2S（GA-A：Pipe `/responses` 拒 `modalities.audio`）、未确认装 Tika / 扩 Direct MIME / 未确认改 Notebook 入口；**不要**把 Image Studio 绑进 OWUI 镜像或把 Studio 钥匙写入 Pipe / `api_configs`；**不要**用薄 Filter 指引冒充已补 Anthropic GitHub API；**不要**把 `$0.05` 工具门当最终账单上限去调高；**未确认不上** Search Controller；**深调研只用 Sonar**，普通气泡不当 Deep Research；搜索费用 **T1 + W6 已落地**（压旧页；OpenAI native + `max_tool_calls=3`）；**W7 / T2 默认不做**（若做：只给非 OpenAI 内轮加次数顶，不做跨「继续」总账）；**M1b 未确认**；**不要**未确认就把 OpenAI 改回 Exa 
