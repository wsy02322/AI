...
| ST-16 | 合格 public 文本挂 `amap_drive_route`（中国）和 `google_drive_route`（海外）：压缩 JSON，每轮最多 3 次。可选 `via`（最多 6 站）回 `legs[]` + `totals`（**M1a 已过门**）。多站中国线再回官方高德链接 + **一张**高德静态示意图（`map_data_uri`；**MAP_LITE**）。海外只要 Google 链接。无交互地图 / 电话 / 评分。失败说「路线接口不可用」。钥匙只在 Tool Valves，禁止回放到聊天。**W1 / W4 / M1a 已过门**。不要和 ST-11/12/14 混号 |
