...
| ST-16 | 合格 public 文本挂 `amap_drive_route`（中国）和 `google_drive_route`（海外）：压缩 JSON，每轮最多 3 次。无地图 UI / 电话 / 评分。失败说「路线接口不可用」。钥匙只在 Tool Valves。**W4 已挂载，海外过门等 Google Maps Key**。不要和 ST-11/12/14 混号 |
| ST-17 | xAI 类薄 Filter Search+Fetch **`engine=native`**（网页+X）。OpenAI 仍 Exa。Google 走 native（W2，记在 ST-14）。Grok 须能引用 x.com。**W5 全模型 X 未确认不上**。不要和 ST-11/12/14/16 混号 |
...
- **已落地**：聊天四格 + 路线 S；ST-14 薄 Web Search（质量**已收口**；**W2 Google native 已过门**）；Live **L1**；**ST-16 高德路线 Tool 已过门**；**ST-17 Grok native / X 已过门**  
