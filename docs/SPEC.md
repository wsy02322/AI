...
| ST-17 | xAI 类薄 Filter Search+Fetch **`engine=native`**（网页+X）。OpenAI 仍 Exa。Google 走 native（W2，记在 ST-14）。Grok 须能引用 x.com。**W5**：官方 X API 按次薄工具 `x_recent_search` 已挂 12 个 public 文本；Key 在 Valves（未注入则过门未过）。失败说「X 接口不可用」，禁止编 `x.com/status`。不要和 ST-11/12/14/16 混号 |
...
- **已落地**：聊天四格 + 路线 S；ST-14 薄 Web Search（质量**已收口**；**W2 Google native 已过门**）；Live **L1**；**ST-16 高德 + Google Routes 已过门**；**ST-17 Grok native / X 已过门**；**W5 `x_recent_search` 已挂（过门等 Key）**  
