...
| ST-16 | 合格 public 文本挂 `amap_drive_route`（中国）和 `google_drive_route`（海外）：压缩 JSON，每轮最多 3 次。可选 `via`（最多 6 站）回 `legs[]` + `totals`（**M1a 已过门**）。多站中国线：每段一条高德网页导航（网页 URI 最多 1 个途经点）+ **一张**沿路网折线的高德静态图（`map_data_uri`；**MAP_LITE / MAP_ROAD / NAV_LEGS / NAV_WEB_ONLY**）。**不**出 `amapuri` / `iosamap` 全程深链（模型会改写/截断）。海外只要 Google 链接。无交互地图 / 电话 / 评分。失败说「路线接口不可用」。钥匙只在 Tool Valves，禁止回放到聊天。**W1 / W4 / M1a 已过门**。**M1b**（偏高速/少收费）未确认。不要和 ST-11/12/14 混号 |
...
- **已落地**：聊天四格 + 路线 S；ST-14 薄 Web Search（质量**已收口**；**W2 Google native / W6 OpenAI native 已过门**）；Live **L1**；**ST-16 高德 + Google Routes + M1a via/legs + MAP_LITE（表+每段网页导航+一张高德静态图；多站不出 App 深链）**；**ST-17 Grok native / X 已过门**；**W5 `x_recent_search` 已过门**  
