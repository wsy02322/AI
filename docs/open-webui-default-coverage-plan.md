...
> **状态**：**C0 曾记「同意」**（2026-09-07）；用户随即要求复述，**待看懂后复认或改口**。未执行。不改实例。  
> **C0 白话**：只同意「以后管道类功能不要写死模型 id」；不是开工，也不是批准中国三只挂搜索。  
> **下一问**：先复认 C0，再谈 D1。
...
### 0.1 C0 白话（你点「同意」时实际只锁了这个）

**C0 是原则，不是施工单。**

你当时点头的意思，按我的推荐，只有四句：

1. 压缩这种「改请求 / 改旧网页」的护栏，按 **有没有这类内容** 做，不要写成「只对 Grok/Sol/Astra 这 9 个 id」。
2. 以后新进聊天列表的 **同类文本模型**，这类护栏应自动生效，不用再改名单。
3. **不是** 给图像、Sonar、视频联网；**不是** 把 OpenRouter 几百个模型都公开。
4. 谁出现在模型列表里，仍要单独点头（选品）。

**C0 不是：**

- 现在就改网站 / Pipe / 搜索开关；
- 现在就让 DeepSeek / Kimi / Qwen 能搜（那是以后的 **C2**，还没问）；
- 现在就跑 T0 探针（那是 **D1**）；
- 选「顶级 C」整包（搜索改 deny + 改 Banner + 写入 AGENTS）。

**是不是最推荐的？** 是。在「质量不降、又不要死名单」里，C0 这四句是我最推荐的底。比它更大的「顶级 C」（连搜索也改成自动挂所有合格文本）我推荐作 **以后** 的终态，不推荐跟 C0 一次做完。

看懂后请复认：`C0：仍同意` 或 `C0：撤回` / 改其中一句。
...
| 轮 | 只问 | 点头之后 | 不点头 |
|----|------|----------|--------|
| **C0** | 原则：覆盖 = **合格类 + 未来同类**；压缩无名单；「所有」≠ 图像/Sonar/OR 全库；选品名单保留 | 2026-09-07 曾记同意；**待复认** | 可撤回 |
