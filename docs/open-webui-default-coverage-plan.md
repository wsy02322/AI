...
> **状态**：**未确认**。只分析和 plan，**不改**实例 / Pipe / Filter / Banner / public。  
> **来源**：费用护栏讨论里「所有模型 = 模型库全部 + 后续新增；其他功能在不明显增加复杂度时也应默认全覆盖」。  
> **决策方式**：一轮只确认一件。本轮只问 **C0**（与费用 plan **D0-cover** 同一问，只点一次头）。
...
**本轮只确认 C0。** 不等于批准改 Filter，也不等于批准 T0 探针。
...
| 轮 | 只问 | 点头之后 | 不点头 |
|----|------|----------|--------|
| **C0（本轮）** | 原则：覆盖 = **合格类 + 未来同类**；压缩无名单；「所有」≠ 图像/Sonar/OR 全库；选品名单保留 | 写入本文件 + 费用 plan 的 T1 约束 | 继续只文档 |
| **C1** | 与费用 **D0/D1** 对齐：T1 是否按「全请求、无名单」设计（建议：是） | 进 T0/T1 设计，禁止按 9 个 id 分支 | 压缩仍可做，但会再漏新模型 |
...
C0 **不等于** C2，也不等于费用 T0。可以：C0 通过 → 费用走 T−（压缩无名单）→ 搜索改类覆盖放到 C2。

建议本轮回复：`C0：同意` 或指出要改的那句（例如「中国三只先不搜」）。
