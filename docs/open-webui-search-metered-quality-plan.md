...
> **状态**：**约束已更新，施工以新 plan 为准**（2026-09-07）。用户改为：日常平均花费对标官网顶级；`$19` 类 spike 使平均抬升约 ≤ `$0.2` 可接受。**不要**再执行本文「生产永远禁止 native / `$19` 概率必须为零」。顶级档施工单见 **`docs/open-webui-search-quality-top-plan.md`**。本文保留 T1/`$19` 根因。  
