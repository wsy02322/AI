...
1. **地图**：地点坐标 + 路网 + 路线 API + 路况；**1 公里精度够**；不要评分、电话。默认 **不嵌交互地图**。多站中国线：**分段表 + 高德导航链接 + 一张高德静态示意图**（§24）。海外：表 + Google 链接，无静态图。单段默认不出静态图。
2. **X 上的内容**。  
...
**W0–W6 已过门**。**M1a 已过门**（§23）。**MAP_LITE 出图已确认**（§24）。**不要自行开 W7**（§21）。**不要自行开 M1b**。
...
**一句话**：模型一次调用，传入有序城市/景点列表；工具回 **每一段** 的公里、分钟、路况大意 + 全程合计。尺子用 M1a。出图另见 **§24**（用户已确认只要第一张高德静态图）。
...
**过门通过。** 出图见 **§24**。M1b / 查店 **未做**。W7 仍见 **§21**。

---

## 24. MAP_LITE 出图（2026-09-07）

用户确认：按 example，**两张图只要第一张**。这是建议主线，不是凑合档。
...
回滚：工具 content 回到 M1a（无 `nav_url` / `map_data_uri`）；Valves Key 不动。
