...
| 即时搜 W1 | **2026-09-07**：Tool `amap_drive_route` 挂 12 public 文本；Flash 能调工具；**无高德 Key** → 正文「路线接口不可用」。`verify_amap_drive_route.py` 15 ok；`verify_stack` 24 ok（`VERIFY_SMOKE=0`）。搜索引擎仍 Exa |
