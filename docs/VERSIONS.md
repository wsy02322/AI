...
| 即时搜 W4 | **已安装未过门**（2026-09-07）：Tool `google_drive_route` 挂 12 文本；`GOOGLE_MAPS_KEY` **未注入**。国内高德回归仍绿（36.7km / 畅通）。`verify_google_drive_route` 17 ok；`verify_stack` 24 ok；Pipe 仍 `9c4836ace251` |
