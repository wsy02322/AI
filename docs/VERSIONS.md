...
见 `AGENTS.md` → **Pipe 更新 Runbook**。更新后重填本表。重放 S2′：`python3 scripts/patch_pipe_cross_model_reasoning.py`。重放 ST-11：`python3 scripts/patch_pipe_fable_thinking_replay.py`（marker 已在则 no-op）。重放 T1 压页：`python3 scripts/patch_pipe_search_page_compact.py`（已有 `SEARCH_PAGE_COMPACT_V1` 则 no-op）。重放 Sol 400 重试：`python3 scripts/patch_pipe_server_tool_fail.py`（已有 `SERVER_TOOL_FAIL_RETRY_V1` 则 no-op）。
