...
> **范围（本版）**：质量不降；覆盖原则见 §2.7（管道默认全模型 + 未来新增，不是死名单）。  
> **决策方式**：关键点分多轮对话，一轮只确认一件。本轮只问 **D0-cover**（§5）。  
...
**本轮只需确认 D0-cover**（§2.7 / §5）：管道类默认全覆盖 + 未来新增；加能力 / picker 仍要 deny 或确认。通过后才谈 T0。
...
|----|------------|------------------|--------------|
| **D0-cover（本轮）** | 接受 §2.7：**A 类默认全可聊模型 + 后续新增、无 id 名单**；T1 必须是 A；B/C 不混进本波；ST-14 改 deny **另门**；质量不降；S1/S2/S4 不进主线 | 写进本文件。T / T− 可本轮顺带说，或下一轮再说 | 继续只文档 |
...
**不要**在 D0-cover 里批准 T0，也不要顺带批准中国三只挂搜索。

建议回复（本轮）：`D0-cover：同意`。若终态也已有意向，可加 `T−` 或 `T`；没有也可以下一轮再说。
