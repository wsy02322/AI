...
> **状态**：**D0-cover / C0 待复认**（2026-09-07 曾记同意，用户要求先看懂）。未执行。  
> **C0 白话**：只锁「压缩按内容、不按模型 id」；不是开工，不是中国三只挂搜。  
> **下一问**：先 `C0：仍同意` 或撤回，再谈 D1。  
...
|----|------------|------------------|--------------|
| **D0-cover** | 接受 §2.7：**A 类默认全可聊模型 + 后续新增、无 id 名单**；T1 必须是 A；B/C 不混进本波；ST-14 改 deny **另门**；质量不降；S1/S2/S4 不进主线 | 2026-09-07 曾记同意；**待复认** | 可撤回 |
