...
| **略降级 S** | Astra / Astra Pro **仍挂** Search，**新对话默认关** | 要路况再开；写作仍是旗舰 | 只改 `defaultFilterIds` + Banner 一句 | 挡住「随手开着搜、说继续」；不修 Sol/Grok 同类滚上下文 |
...
**顶级（建议作主线）：T0 → T1 → T2**，T3 随 T1 一起做。Astra 搜索保持 default-on。质量目标：同一条「你继续」仍能写完行程，只是不再把旧整页当新上下文。

**略降级、当天能落地：S1 + Banner 一句**（例如 v7：Astra 可搜，新对话默认关，要查再开）。Grok/Sol/Claude/Gemini 仍是 ChatGPT 那种默认会搜。
