...
> **范围（本版）**：质量不降；**全部**会滚 Search/Fetch 整页的模型（现网 = ST-14 共 9 个）。  
> **决策方式**：关键点分多轮对话，一轮只确认一件。本轮只问 **D0**（§5）。  
...
**本轮只需确认 D0**（§5）。D0 通过后才谈 T0 探针；T0 看完再谈切哪一层、摘录留多长、T2 做不做。
...
1. **T / T− 做一次、覆盖全部 ST-14。** 出站压缩在 Pipe，不是 Astra 专用补丁。为 Astra 单独写一套、让 Opus/Fable/Sol 继续能滚 495 万，是假顶级。
2. **S1 默认关搜索不进「质量不降」主线。** 以后若另点头，也只动 Astra 一对，不动另外 7 个。
3. **不要**为另外 7 个单独拧全局阀门，也不要给它们各做一套 Filter。
4. **不在 ST-14 上的模型不用本护栏。** DeepSeek / Kimi / Qwen 现在不能搜，没有这条失败模式。以后若挂上 Search，自动吃同一层 Pipe 压缩。Sonar 是自己的搜索计价，不是整页回放。图像 / 视频无此路径。

Grok 还有「总 token 超 200k 加价」档（OpenRouter 页）。即使单价是 Astra 的 1/5，整页滚起来仍会到数美元。T1 把 prompt 压回十万以内，对这些模型同样值。

### 2.7 以后加模型会不会默认吃到这套

**会，但必须按「有工具结果就压」设计，不能再做一份模型名单。** 现网还没有 T1；下面是落地后的继承规则，D2 写补丁时要钉死。

| 以后发生的事 | 会不会自动搜 | 会不会自动压旧整页 |
|--------------|--------------|-------------------|
| 新文本进 public，**不**挂薄 Filter | 否 | 无工具页，压缩空转 |
| 新文本挂上 ST-14（进 Filter allowlist + attach + refresh） | 是（与现 9 个同一套阀门） | **是**——Pipe 看出站里已有 Search/Fetch 旧项就压，**不**看模型 id |
| 中国三只（DeepSeek / Kimi / Qwen）以后挂搜索 | 是 | 是（同一层） |
| 换代：只改 retained-family 最新 id，仍挂同一 Filter | 是 | 是 |
| Sonar / 图像 / 视频 | 否（Guard 剥 tools） | 无此路径 |
| 新模型走别的 Pipe / 直连 `openai.api_configs` | 不在本栈 | **不会**（本护栏只活在现网 OpenRouter Pipe） |

因此：**加模型的人不必再记一条「别忘了费用白名单」。** 该记的只有 ST-14 挂载（能不能搜）。费用护栏是 Pipe 出站的默认行为。

反例（禁止）：再写 `COST_GUARD_MODEL_IDS` / `if "astra" in model`。Astra 进搜索时已经证明——名单漏一只，门就当没装。压缩按 **item 类型**（旧 `web_search` / `web_fetch` 整页），不按模型。

T2 若做，同样按「这次用户消息的工具累计」，不按模型。新旗舰默认同一预算。

未落地前：新模型挂上 ST-14 = 马上能搜，也马上能滚整页。**没有**第三道自动护栏。这就是 Astra 一对挂上之后，同一套阀门直接把 `$19` 放进来的原因。
...
1. **默认搜索仍开。** 9 个 ST-14 新对话仍 default-on。隐含时效仍会搜（现有 Search 烟雾 + EVAL-B 形态）。
...
| **T1** | 出站前把 **上一轮及更早** 的 Search/Fetch **整页**压成标题 + URL + 日期/数字 + 一两句；**本轮**整页保留 | ChatGPT 类；当前轮仍真读页；来源 URL 还在 | 压太狠会丢数字。摘录规则在 T0 之后单独确认（D2） |
...
|----|------------|------------------|--------------|
| **D0（本轮）** | 主线 = **全部 9 个 ST-14** + **质量不降** + 走 **T 或 T−**（S1/S2/S4 不进主线） | 写进本文件为已选范围；下一轮才提 T0 | 继续只文档，不发探针 |
| **D1** | 是否跑 **T0** 只读探针（真请求，几分钱；不改 Pipe 业务、不留 marker） | 执行 T0，把回放形态写回本文件 | 不碰实例 |
| **D2** | T0 看完：切哪一层；摘录留什么（建议：标题 + URL + 日期/数字 + ≤2 句）；**按 tool item 压、不按模型名单**；T− 还是直接排 T2 | 才写 T1 补丁设计 / 单测夹具 | 不改 Pipe |
| **D3** | T1 绿了之后：T2 做不做；若做，累计上限用「仍 `$0.05` 总量」还是「Search 次数 / Fetch 次数」 | 才改跨轮记账 | T− 收口，T2 另开一轮 |
| **D4** | 验收名单：最低 Sol + Astra Pro「短搜→继续」对照；是否加 Opus/Fable/Grok 各一条 | 按名单跑，红则回滚 | 不扩大评测账单 |

**不要**在 D0 里顺带批准 T0。T0 会打现网推理。

建议回复（本轮）：`D0：T−` 或 `D0：T`。意思是范围接受，终态要不要跨轮预算可以到 D2/D3 再定；选 `T` 只表示终态意向，**仍先做 T0**。
...
| 动作 | 过门 |
|------|------|
| Pipe **content-only** 压缩旧 Search/Fetch；本轮整页保留；超 ~200k 先压。不碰 valves / `API_KEY`。单测用假 tool item | 续聊 prompt 少一个数量级；可见答案仍有 URL 和关键数字；Sol + Astra Pro Search+Fetch 烟雾仍绿；ST-10/ST-11 探针不回退 |
...
- 未确认改实例 / Pipe / Filter / Banner。D0 不等于批准 T0。
