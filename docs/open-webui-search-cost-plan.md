...
> **状态**：**D0-cover / C0 已确认**（2026-09-07）。原则锁定，**未执行**。不改实例 / Pipe / Filter / Banner。  
> **范围（本版）**：质量不降；覆盖原则见 §2.7（管道默认全模型 + 未来新增，不是死名单）。  
> **下一问**：**D1** — 是否跑 T0 只读探针（真请求，几分钱；不改 Pipe 业务、不留 marker）。  
...
|----|------------|------------------|--------------|
| **D0-cover** | 接受 §2.7：**A 类默认全可聊模型 + 后续新增、无 id 名单**；T1 必须是 A；B/C 不混进本波；ST-14 改 deny **另门**；质量不降；S1/S2/S4 不进主线 | **已确认 2026-09-07** | — |
